...
bool Application::init(const int& width, const int& height) {
	...
	//3.设置当前窗体对象为OpenGL的绘制舞台
	glfwMakeContextCurrent(mWindow);
	if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
		std::cout << "Failed to initialize GLAD" << std::endl;
		return false;
	}
	//将当前 Application 实例（this）的指针存储到 GLFW 窗口的“用户指针”中。
	//在静态回调函数（如 frameBufferSizeCallback）中，可以通过 glfwGetWindowUserPointer 获取 Application 实例，进而访问成员变量（如 mResizeCallback）
...
void Application::getCursorPosition(double* x, double* y) {
	glfwGetCursorPos(mWindow, x, y);//获取传入窗体的鼠标位置
}
